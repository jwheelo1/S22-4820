...
-- -- from: ./county/what_counties_voted_for_joe.sql
...
---  	county_name			text,
---  	state				text,
--- 	winner				text -- either 'Joseph R Biden Jr' or 'Donald J Trump'
--- );
--- 
...
select t4.winner,
		lpad(round((t4.sum_growth::float/(select sum(t4.sum_growth::float) from t4))::numeric*100,2)::text||'%',13,' ') pct_of_growth
	from t4
;
